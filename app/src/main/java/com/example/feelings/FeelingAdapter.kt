...
class FeelingAdapter internal constructor(context: Context) : RecyclerView.Adapter<FeelingAdapter.FeelingViewHolder>() {
    ...
    override fun onBindViewHolder(holder: FeelingViewHolder, position: Int) {
        ...
        if(feeling.mood == 1) {
            holder.imageViewFeeling.setImageResource(R.drawable.ic_sentiment_sad_yellow_24dp)
        }else if(feeling.mood == 2) {
            holder.imageViewFeeling.setImageResource(R.drawable.ic_sentiment_neutral_black_24dp)
        }else {
            holder.imageViewFeeling.setImageResource(R.drawable.ic_sentiment_happy_yellow_24dp)
        }
    ...
    inner class FeelingViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        val imageViewFeeling: ImageView = itemView.findViewById(R.id.imageView)
